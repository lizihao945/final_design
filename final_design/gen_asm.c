 ...
 */
void asm_arg_str_with_ebx(t_quad_arg arg, struct asm_arg_st *asm_arg) {
	char *tmp;
	char *s;
...
void cancel_allocation(int idx) {
	struct asm_arg_st *arg;
	arg = (struct asm_arg_st *) malloc(sizeof(struct asm_arg_st));
	arg->name = (char *) malloc(sizeof(char) * 256);
	asm_arg_str_with_ebx(allocation[idx].arg, arg);
	fprintf(out, "\tmov %s, %s\n", arg->name, reg_name[allocation[idx].reg_idx]);
	// easy way to make this allocation unavailable
	allocation[idx].reg_idx = -1;
	allocation[idx].arg.arg_code = 0;
}
...
int mov_quad_arg_to_reg(t_quad_arg arg) {
	int tmp, rt;
	struct asm_arg_st *asm_arg;
	if ((tmp = allocated(arg)) != -1)
		return tmp;
	asm_arg = (struct asm_arg_st *) malloc(sizeof(struct asm_arg_st));
	asm_arg->name = (char *) malloc(sizeof(char) * 256);
	asm_arg_str_with_ebx(arg, asm_arg);
	rt = get_reg_idx_to_allocate(arg);
	fprintf(out, "\tmov %s, %s\n", reg_name[rt], asm_arg->name);
	return rt;
}
...
void gen_jcc_with_temp_reg_all(int flag) {
	...
	// quadruple[quad_idx] is a cmp inst.
	asm_arg_str_with_ebx(quadruple[quad_idx].arg1, arg1);
	arg_idx1 = get_reg_idx_to_allocate(quadruple[quad_idx].arg1);
	fprintf(out, "\tmov %s, %s\n", reg_name[arg_idx1], arg1->name);
	asm_arg_str_with_ebx(quadruple[quad_idx].arg2, arg2);
	arg_idx2 = get_reg_idx_to_allocate(quadruple[quad_idx].arg2);
	fprintf(out, "\tmov %s, %s\n", reg_name[arg_idx2], arg2->name);
	fprintf(out, "\tcmp %s, %s\n", reg_name[arg_idx1], reg_name[arg_idx2]);
	asm_arg_str_with_ebx(quadruple[quad_idx+1].arg1, arg1);
	switch (flag) {
		case QUAD_NEQL:
			fprintf(out, "\tje %s\n", arg1->name);
			break;
	...
	for (quad_idx = 0; quad_idx < quadruple_top; quad_idx++) {
		...
		switch(quadruple[quad_idx].op) {
			case QUAD_ASSIGN:
				...
				break;
			case QUAD_ADD:
				reg_idx1 = mov_quad_arg_to_reg(quadruple[quad_idx].arg1);
				if ((tmp = allocated(quadruple[quad_idx].arg2)) != -1)
					fprintf(out, "\tadd %s, %s\n", reg_name[reg_idx1], reg_name[tmp]);
				else {
					asm_arg_str_with_ebx(quadruple[quad_idx].arg2, arg2);
					fprintf(out, "\tadd %s, %s\n", reg_name[reg_idx1], arg2->name);
				}
				asm_arg_str_with_ebx(quadruple[quad_idx].result, arg3);
				fprintf(out, "\tmov %s, eax\n", arg3->name);
				break;
			case QUAD_SUB:
				reg_idx1 = mov_quad_arg_to_reg(quadruple[quad_idx].arg1);
				...
				allocation[allocation_top].reg_idx = allocated(quadruple[quad_idx].arg1);
				break;
			case QUAD_MULT:
				asm_arg_str_with_ebx(quadruple[quad_idx].arg1, arg1);
				fprintf(out, "\tmov eax, %s\n", arg1->name);
